...
- Proxy-aware HTTP: trust only known proxy addresses and select the authoritative forwarding header; see [HTTP hardening](harden-http.md). `X-Forwarded-For` is the default; `Forwarded` requires explicit selection.
